...
<div class = "row">
        <div class ="col-sm-12">
            <div class="jumbotron jumbotron-fluid">
                <div class="container">
                    <p class="lead">Admin only Access - List of Trainers</p>
                </div>
            </div>
        </div>
</div>
    ...
    <div id = "wrapper">
            ...
            <input type="button" class="btn btn-secondary" value="Add Trainer"
                   onclick="window.location.href='pokazDodajTreneraForm';return false;">
            <hr>
        ...
        <table class="table table-bordered">
           ...
           <th></th>
               ...
               <c:forEach var="tempTrener" items="${myTrenerzy}">
                   <c:url var ="updateLink" value ="/pokazTrenerFormUpdate">
                       <c:param name="trenerEmail" value = "${tempTrener.email}" />
                   </c:url>

                   <c:url var ="deleteLink" value ="/usunTrenera">
                       <c:param name="trenerEmail" value = "${tempTrener.email}" />
                   </c:url>

                   <tr>
                       <td>${tempTrener.imie}</td>
                       <td>${tempTrener.nazwisko}</td>
                       <td>${tempTrener.pensja}</td>
                       <td>${tempTrener.email}</td>
                       <td><a href = "${updateLink}" class="btn btn-secondary">Update</a></td>
                       <td><a href = "${deleteLink}" class="btn btn-danger"
                       onclick="if(!(confirm('Are you sure you want to delete this Trainer?'))) return false">Delete</a></td>
                     </tr>
               </c:forEach>
