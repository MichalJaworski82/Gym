...
<div class = "row">
        <div class ="col-sm-12">
            <div class="jumbotron jumbotron-fluid">
                <div class="container">
                    <p class="lead">Admin only Access - List of Clients</p>
                </div>
            </div>
        </div>
</div>
    ...
    <div id = "wrapper">
            ...
            <input type="button" class="btn btn-secondary" value="Add Client"
                   onclick="window.location.href='pokazDodajKlientaForm';return false;">
            <hr>
        ...
        <table class="table table-bordered">
           ...
           <th></th>
               ...
               <c:forEach var="tempKlient" items="${myKlienci}">
                   <c:url var ="updateLink" value ="/pokazKlientFormUpdate">
                       <c:param name="klientEmail" value = "${tempKlient.email}" />
                   </c:url>

                   <c:url var ="deleteLink" value ="/usunKlienta">
                       <c:param name="klientEmail" value = "${tempKlient.email}" />
                   </c:url>

                   <tr>
                       <td>${tempKlient.imie}</td>
                       <td>${tempKlient.nazwisko}</td>

                       <td>${tempKlient.email}</td>
                       <td><a href = "${updateLink}" class="btn btn-secondary">Update</a></td>
                       <td><a href = "${deleteLink}" class="btn btn-danger"
                       onclick="if(!(confirm('Are you sure you want to delete this Client?'))) return false">Delete</a></td>
                     </tr>
               </c:forEach>
